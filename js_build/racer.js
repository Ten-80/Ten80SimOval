...
function drawTrack() {
    ...
    scale = usableW / xmax;
    document.getElementById('canvas').height = ymax * scale + trackC.y + trackC.x;
    document.getElementById('totemCanvas').height = ymax * scale + trackC.y + trackC.x;
    let usableH = document.getElementById('canvas').height - 20;


    let rankS;
    // [['#2060ff', '#fff', 1, 1], ['#ff4000', '#fff', 1, 2], ['#ffffff', '#000', 1, 3], ['#008000', '#000', .1, 0]].forEach((color, idx) => {
    // [['#2060ff', '#fff', 1, 1], ['#ff4000', '#fff', 1, 2], ['#ffffff', '#000', 1, 3]].forEach((color, idx) => {
...
function buildPitLine(racer, lapIdx) {
    ...
    let numInPts = 250;
    let pitPt = [racer.line[190][0], racer.line[190][1]];
    pitPt[1] = 1.5;

    let totalInX = pitPt[0] - handoffX;
    let intoLine = calculateLinePts(handoffX, handoffY, pitPt[0], pitPt[1], numInPts);
    let curveIntoPit = calculateEdgePts([pitPt[0], 0], 1.5, 90, -180, 24);
    let curveOutPit = calculateEdgePts([27, 0], 2, 270, -90, 18);
    let curveIntoTrack = calculateEdgePts([27, 0], 2, 180, -90, 18);
    ...
    let pitFromPark = calculateLinePts(
        ...
        numPitPts / 3 * ((pitX - 27) / distToExit)
    );
...
function setRacerOnLine(team, gridPos) {
    ...
    // let color = ['#80f0ff', '#00a040', '#ff2000', '#80ff00', '#0040cf', '#ffa0ff',
    //     '#6000a0', '#ff8000', '#ffff00', '#ff00c7'][gridPos];
    // let invcolor = ['#000', '#fff', '#fff', '#000', '#fff', '#000',
    //     '#fff', '#fff', '#000', '#fff'][gridPos];
    let color = [
        '#ff0000', '#209420', '#ffe100', '#0000ff', '#f58231',
        '#30c0d0', '#b0f0ff', '#bfef45', '#ff00c7', '#facee4',
        '#911eb4', '#8a8300'][gridPos];
    let invcolor = ['#fff', '#fff', '#000', '#fff', '#fff', '#fff',
        '#000', '#fff', '#000', '#000', '#fff', '#fff'][gridPos];

    let pix = scale;
    outlineS.graphics.setStrokeStyle(1);
    outlineS.graphics.beginStroke('#000');
    outlineS.graphics.drawRect(-scale, -scale / 2, scale * 2, scale);

    carS.graphics.beginFill(color);
    carS.graphics.drawRect(-scale, -scale / 2, scale * 2, scale);
    ...
    let labelTf = new createjs.Text(team.id, "12px Helvetica", "#fff");
    ...
    labelS.graphics.drawCircle(9, 8, 7);
    ...
    pitS.graphics.lineTo(0, -scale * 2);
    pitS.graphics.beginFill('#000');
    pitS.graphics.drawRect(0, -scale * 2.5, scale * 3.5, scale * 1.4);
    pitS.graphics.endFill();
    let flag = new createjs.Container();
    flag.addChild(pitS);
    pitTf.x = scale * 2 + 2;
    pitTf.y = -scale * 2.5 + 1;//(scale * 1.3 - pitTf.getMeasuredHeight()) / 2;
    flag.addChild(pitTf);
    pitLabel = flag;
    ...
    let startY = (startingLine[3] - startingLine[1] - precalculatedRacingLines.length) / 2 + lineIdx;
    ...
    console.log(team.id, line.length, lineIdx);
